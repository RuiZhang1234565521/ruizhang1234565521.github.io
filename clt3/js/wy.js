...
async function getdb(x){
    ...
    if(x==1){
        ...
        db0.sql = `select id,time,data1 from ${data.dbname} where time >= '${date1}' and time <= '${date2}' order by time limit 5000`
    }
    if(x==2){
        let date1 = data.year0 + '-01-01'
        let date2 = data.year0 + '-12-31 23:59:59'
        db0.sql = `select id,time,data1 from ${data.dbname} where time >= '${date1}' and time <= '${date2}' and data1 like '%${data.ssstr}%' collate utf8_general_ci order by time limit 500`
    }
    a.innerHTML=db0.sql
...
async function baocun(x){
    ...
    let db0 = {mode:1,opid:data.opid}
    if(x==1){db0.sql = `insert into ${data.dbname} (id,time,data1,data2) values ('${id}','${time}','${data1}','${data2}')`}
    if(x==2){db0.sql = `update ${data.dbname} set time='${time}',data1='${data1}',data2='${data2}' where id=${id}`}
...
async function dbadd(){
    let db0 = {mode:1,opid:data.opid}
    db0.sql = `select id from ${data.dbname} order by id desc limit 1`
    a.innerHTML=db0.sql
    let db = await postdb(db0)
    if(db){id = +db[0].id + 1}else{id = 1}
...
async function xiugai(e){
    ...
    let db0 = {mode:1,opid:data.opid}
    db0.sql = `select id,time,data1,data2 from ${data.dbname} where id = ${id}`
    a.innerHTML=db0.sql
    let db = await postdb(db0)
...
async function shanchu2(id){
    dog.close()
    let db0 = {mode:1,opid:data.opid}
    db0.sql = `DELETE FROM ${data.dbname} WHERE id = ${id}`
...
async function postdb(db1) {
    ...
    const url = 'http://171.80.8.90/myapi/webapi2.php';
    try {
